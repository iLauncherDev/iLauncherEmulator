...
start:
    ...
    {
    case 0x04:
        x86_write_reg(cpu, x86_reg_al, x86_read_reg(cpu, x86_reg_al) - (int8_t)x86_simm(cpu, x86_reg_ip, 1));
        break;
    case 0x0f:
...
cpu_t *x86_setup()
{
    ...
    for (uint16_t i = 0; i < 256; i++)
    {
        if (i >= x86_reg_al && i <= x86_reg_bh)
            reg_x86[i] = ((i - x86_reg_al - (i & 0x01)) << 1) + (i & 0x01), reg_x86_size[i] = 1;
