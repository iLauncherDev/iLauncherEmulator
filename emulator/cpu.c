...
static inline void cpu_push_reg(uint8_t stack, uint8_t reg, uint8_t size)
{
    ...
    {
    case 1:
        *((uint8_t *)&vm_memory[cpu_state[stack]]) = cpu_read_reg(stack) & 0xff;
        break;
    case 2:
        *((uint16_t *)&vm_memory[cpu_state[stack]]) = cpu_read_reg(stack) & 0xffff;
        break;
    case 4:
        *((uint32_t *)&vm_memory[cpu_state[stack]]) = cpu_read_reg(stack) & 0xffffffff;
        break;
    default:
        *((uint64_t *)&vm_memory[cpu_state[stack]]) = cpu_read_reg(stack);
        break;
    }
}
...
void cpu_emulate_i8086(uint8_t debug)
{
    ...
    char *operation;
    if (debug)
        printf("IP: 0x%x, OPCODE 0x%x: ", (uint16_t)cpu_read_reg(cpu_reg_ip), *opcode);
    switch (*opcode)
    {
    case 0x06:
    ...
    case 0xcd:
        ...
        else if (value2 == 0x20)
        {
            for (uint32_t i = window_framebuffer[0];
                 ...
                 i++)
                vm_memory[i] = cpu_read_reg(cpu_reg_ax);
        }
        if (debug)
